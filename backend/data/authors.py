...
def __init__(self, author_name="NaN", author_birth_date="NaN", author_death_date="NaN", author_top_work="NaN", author_work_count=0, author_bio="NaN", author_image="NaN"):
...
def load_file_position():
    with open('position.txt','r') as file:
        text_and_line = (lists.index(file.readline().replace('\n','')), int(file.readline()))
        ...
        for y in range(int(position[1]), len(fulltext)):
            ...
            if data['numFound'] == 0:
                ...
                if item['name'] not in author_set:
                    ...
                    request_img_url = 'https://en.wikipedia.org/w/api.php?action=query&format=json&prop=pageimages&titles=' + str + '&piprop=original'
